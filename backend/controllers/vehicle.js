...
export const addVehicle = async (req, res) => {
  ...
  newVehicle = await newVehicle
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Coludn't add the vehicle", error: err.message });
    });
};
...
export const viewVehicles = async (req, res) => {
  await Vehicle.find()
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with retrieving data", error: err.message });
    });
};
...
export const editVehicle = async (req, res) => {
  ...
  newVehicle = await Vehicle.findByIdAndUpdate(id, newVehicle)
    .then((newVehicle) => {
